...
class BDDReachability:
    ...
    def compute_reachable_bdd(self):
        """
        Tính tập reachable markings ở dạng BDD:
        S_{k+1}(x) = S_k(x) ∪ Post(S_k)(x)
                   = S_k(x) ∪ ∃x. ( S_k(x) ∧ R(x,x') )[x'→x]
        """
        # S ban đầu: chỉ có initial marking
        S = self.marking_to_bdd(self.net.initMarking)

        # Các biến cần khử ∃: tên biến 'x0', 'x1', ..., 'x{n-1}'
        ...
        while True:
            ...
            # img(x') = ∃x. tmp(x, x')
            # Ở đây dùng tên biến (string) cho exist
            img = self.bdd.exist(quant_vars, tmp)

            # Đổi tên xp_i -> x_i: mapping từ tên 'xp{i}' sang node x_i
    ...
    def enumerate_markings(self, bdd_node, limit=1_000_000):
        """
        Liệt kê các marking thoả BDD node (dạng list 0/1).
        Dùng self.bdd.pick(...) để lần lượt lấy nghiệm và loại dần.
        """
        out = []
        u = bdd_node

        while u != self.bdd.false and len(out) < limit:
            # pick: lấy một nghiệm (model) của u
            model = self.bdd.pick(u)
            if model is None:
                break

            # model là dict: {'x0': 0/1, 'x1': 0/1, ...}
            m = [model.get(f"x{i}", 0) for i in range(self.n)]
            out.append(m)

            # Xây "cube" tương ứng với nghiệm này để loại nó khỏi u:
            cube = self.bdd.true
            for i in range(self.n):
                var_name = f"x{i}"
            ...
            # Loại nghiệm vừa lấy khỏi u: u = u ∧ ¬cube
            u = u & ~cube
